...
void semantic_graph_slam::empty_landmark_queue(std::vector<landmark> current_lan_queue,
                                      ...
                                      Eigen::MatrixXf odom_cov)
{
    ...
    //dont update keyframes if the keyframe time and distance is less or no detection is available
...
void semantic_graph_slam::addFirstPoseAndLandmark()
{
    ...
    {
        for(int i = 0; i < new_keyframes_.size(); ++i)
            empty_landmark_queue(first_lan_vec,
                                 new_keyframes_[i]);
...
void semantic_graph_slam::getKeyframes(std::vector<ps_graph_slam::KeyFrame::Ptr> &keyframes)
{
